...
export async function TrOverWSHandler(request: Request, env: Env): Promise<Response> {
    ...
    const writableStream = new WritableStream({
        async write(chunk, _controller) {
            ...
            if (remoteSocketWapper.value) { const writer = remoteSocketWapper.value.writable.getWriter(); try { await writer.write(chunk); } finally { writer.releaseLock(); } return; }
            const { hasError, message, portRemote = 443, addressRemote = "", rawClientData, user } = await parseTrHeader(chunk, env);
            address = addressRemote; portWithRandomLog = `${portRemote}--${Math.random()} tcp`;
            if (hasError) throw new Error(message);
            if (user) {
                usageGuard = new UserUsageGuard(user, env, webSocket); await usageGuard.start();
    ...
    readableWebSocketStream.pipeTo(writableStream).catch(error => { log("readableWebSocketStream pipeTo error", error); safeCloseTcpSocket(remoteSocketWapper.value); void usageGuard?.close(); });
...
async function parseTrHeader(buffer: ArrayBuffer, env: Env) {
    ...
    const crLfIndex = 56, cr = new Uint8Array(buffer.slice(crLfIndex, crLfIndex + 1))[0], lf = new Uint8Array(buffer.slice(crLfIndex + 1, crLfIndex + 2))[0];
    ...
    if (user) { if (getStatus(user) !== 'active') return { hasError: true, message: `user ${getStatus(user)}` }; }
    else { const { TrPass } = globalThis.globalConfig; if (password !== sha224(TrPass!)) return { hasError: true, message: "invalid password" }; }
    const socks5DataBuffer = buffer.slice(crLfIndex + 2); if (socks5DataBuffer.byteLength < 6) return { hasError: true, message: "invalid SOCKS5 request data" };
    const view = new DataView(socks5DataBuffer); const cmd = view.getUint8(0); if (cmd !== 1) return { hasError: true, message: "unsupported command, only TCP (CONNECT) is allowed" };
    const atype = view.getUint8(1); let addressLength = 0, addressIndex = 2, address = "";
    switch (atype) { case 1: addressLength = 4; address = new Uint8Array(socks5DataBuffer.slice(addressIndex, addressIndex + addressLength)).join("."); break; case 3: addressLength = new Uint8Array(socks5DataBuffer.slice(addressIndex, addressIndex + 1))[0]; addressIndex += 1; address = new TextDecoder().decode(socks5DataBuffer.slice(addressIndex, addressIndex + addressLength)); break; case 4: { addressLength = 16; const dataView = new DataView(socks5DataBuffer.slice(addressIndex, addressIndex + addressLength)); const ipv6 = []; for (let i = 0; i < 8; i++) ipv6.push(dataView.getUint16(i * 2).toString(16)); address = ipv6.join(":"); break; } default: return { hasError: true, message: `invalid addressType is ${atype}` }; }
    if (!address) return { hasError: true, message: `address is empty, addressType is ${atype}` };
    const portIndex = addressIndex + addressLength, portRemote = new DataView(socks5DataBuffer.slice(portIndex, portIndex + 2)).getUint16(0);
