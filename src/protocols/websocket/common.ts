...
export async function handleTCPOutBound(
    ...
    async function retry() {
        ...
        if (proxyMode === 'proxyip') {
            ...
            addressRemote = host || addressRemote; portRemote = port || portRemote;
        ...
        try {
            ...
            void remoteSocketToWS(tcpSocket, webSocket, VLResponseHeader, null, log).catch(error => console.error('Retry socket pipe failed:', error));
        } catch (error) {
    ...
    try {
        const tcpSocket = await connectAndWrite(addressRemote, portRemote);
        void remoteSocketToWS(tcpSocket, webSocket, VLResponseHeader, retry, log).catch(error => console.error('TCP socket pipe failed:', error));
    } catch (error) {
        console.error(`Connection failed: ${safeErrorMessage(error)}`);
        const { proxyMode } = globalThis.wsConfig;
        if (proxyMode === 'proxyip' || proxyMode === 'prefix') await retry();
        else {
            safeCloseWebSocket(webSocket);
...
async function remoteSocketToWS(remoteSocket: Socket, webSocket: WebSocket, VLResponseHeader: Uint8Array<ArrayBuffer> | null, retry: Function | null, log: Function) {
    ...
    const writableStream = new WritableStream({
        ...
        async write(chunk, controller) {
            ...
            if (webSocket.readyState !== WS_READY_STATE_OPEN) { controller.error("webSocket.readyState is not open, maybe close"); return; }
            if (vlHeader) { webSocket.send(await new Blob([vlHeader, chunk]).arrayBuffer()); vlHeader = null; }
            else webSocket.send(chunk);
        },
        close() { log(`remoteConnection.readable is close with hasIncomingData is ${hasIncomingData}`); },
        abort(reason) { console.error(`remoteConnection.readable abort`, reason); safeCloseTcpSocket(remoteSocket); }
    });
    try { await remoteSocket.readable.pipeTo(writableStream); }
    catch (error) { console.error('VLRemoteSocketToWS has exception.', error); safeCloseTcpSocket(remoteSocket); safeCloseWebSocket(webSocket); throw error; }
    if (hasIncomingData === false && retry) { log(`retry`); await retry(); }
}
...
export function makeReadableWebSocketStream(webSocketServer: WebSocket, earlyDataHeader: string, log: Function) {
    ...
    const stream = new ReadableStream({
        start(controller) {
            ...
            webSocketServer.addEventListener("close", () => { if (!readableStreamCancel) controller.close(); safeCloseWebSocket(webSocketServer); });
...
function base64ToArrayBuffer(base64Str: string) {
    ...
    try { const decode = atob(base64Str.replace(/-/g, '+').replace(/_/g, '/')); return { earlyData: Uint8Array.from(decode, c => c.charCodeAt(0)).buffer, error: null }; }
    catch (error) { return { earlyData: null, error }; }
}

export function safeCloseTcpSocket(socket: Socket | null) { if (socket) { try { socket.close(); } catch (error) { console.error("Failed to close TCP socket:", error); } } }
export function safeCloseWebSocket(socket: WebSocket) { try { if (socket.readyState === WS_READY_STATE_OPEN || socket.readyState === WS_READY_STATE_CLOSING) socket.close(); } catch (error) { console.error('safeCloseWebSocket error', error); } }

async function getDynamicProxyIP(address: string, prefix: string) {
    let finalAddress = address;
    if (!isIPv4(address)) { const { ipv4 } = await resolveDNS(address, true); if (ipv4.length) finalAddress = ipv4[0]; else throw new Error('Unable to find IPv4 in DNS records'); }
    return convertToNAT64IPv6(finalAddress, prefix);
}
function convertToNAT64IPv6(ipv4Address: string, prefix: string) {
    const parts = ipv4Address.split('.'); if (parts.length !== 4) throw new Error('Invalid IPv4 address');
    const hex = parts.map(part => { const num = parseInt(part, 10); if (num < 0 || num > 255) throw new Error('Invalid IPv4 address'); return num.toString(16).padStart(2, '0'); });
    const match = prefix.match(/^\[([0-9A-Fa-f:]+)\]$/); if (match) return `[${match[1]}${hex[0]}${hex[1]}:${hex[2]}${hex[3]}]`;
    return undefined;
}
