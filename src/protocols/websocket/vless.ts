...
export async function VlOverWSHandler(request: Request, env: Env): Promise<Response> {
    ...
    const writableStream = new WritableStream({
        async write(chunk) {
            ...
            const presentedUUID = extractUUID(chunk); if (!presentedUUID) throw new Error("invalid user");
            ...
            if (isUDP) {
                if (portRemote === 53) { isDns = true; const { write } = await handleUDPOutBound(webSocket, VLResponseHeader, log); udpStreamWrite = write; await udpStreamWrite(rawClientData); return; }
...
function parseVlHeader(VLBuffer: ArrayBuffer, userID: string) {
    ...
    const version = new Uint8Array(VLBuffer.slice(0, 1)), slicedBuffer = new Uint8Array(VLBuffer.slice(1, 17)), slicedBufferString = stringify(slicedBuffer);
    if (slicedBufferString !== userID) return { hasError: true, message: "invalid user" };
    const optLength = new Uint8Array(VLBuffer.slice(17, 18))[0], command = new Uint8Array(VLBuffer.slice(18 + optLength, 19 + optLength))[0];
    let isUDP = false; if (command === 2) isUDP = true; else if (command !== 1) return { hasError: true, message: `command ${command} is not supported, command 01-tcp,02-udp,03-mux` };
    const portIndex = 19 + optLength, portRemote = new DataView(VLBuffer.slice(portIndex, portIndex + 2)).getUint16(0), addressIndex = portIndex + 2, addressType = new Uint8Array(VLBuffer.slice(addressIndex, addressIndex + 1))[0];
...
async function handleUDPOutBound(webSocket: WebSocket, VLResponseHeader: Uint8Array<ArrayBuffer>, log: Function) {
    ...
    const writer = transformStream.writable.getWriter(); return { async write(chunk: ArrayBuffer) { await writer.write(chunk); } };
}
