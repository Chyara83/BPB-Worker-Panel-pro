...
import { fallback, serveIcon, renderSecrets, handlePanel, handleSubscriptions, handleLogin, logout, renderError, handleWebsocket, handleDoH, handleProxyIPs, handleUserSub } from '@handlers';
    ...
    async fetch(request: Request, env: Env) {
        try {
            ...
            switch (path) {
                ...
                case 'sub':
                    if (pathName.startsWith('/sub/user/')) return await handleUserSub(request, env);
