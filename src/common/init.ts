...
export async function init(request: Request, env: Env) {
    ...
    let userID = UUID;
    let TrPass = TR_PASS;
    const userSubMatch = decodeURIComponent(pathname).match(/^\/sub\/user\/([^/]+)(?:\/.*)?$/);
    if (userSubMatch) {
        const user = await findUserBySubPath(userSubMatch[1], env);
        if (user && getStatus(user) === 'active') { userID = user.vlessUUID; TrPass = user.trojanPassword; }
    }
    globalThis.globalConfig = { userID, TrPass, pathName: decodeURIComponent(pathname), fallbackDomain: FALLBACK || 'www.hcaptcha.com', dohURL: DOH_URL || 'https://cloudflare-dns.com/dns-query', runtimeEnv: env };
}

export function initWs(env: any) {
    const { _public_proxy_ip_ } = globalThis.dict;
    globalThis.wsConfig = { envProxyIPs: env.PROXY_IP, envPrefixes: env.PREFIX, defaultProxyIPs: [_public_proxy_ip_], defaultPrefixes: ['[2a02:898:146:64::]', '[2602:fc59:b0:64::]', '[2602:fc59:11:64::]'] };
}
...
export function initHttp(request: Request, env: any) {
    ...
    globalThis.httpConfig = { panelVersion: __VERSION__, defaultHttpPorts: [80, 8080, 2052, 2082, 2086, 2095, 8880], defaultHttpsPorts: [443, 8443, 2053, 2083, 2087, 2096], hostName: hostname, client: decodeURIComponent(searchParams.get('app') ?? ''), urlOrigin: origin, subPath: SUB_PATH || UUID };
}
